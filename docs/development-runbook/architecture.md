...
- 云端到本地的 HTTP 通信必须同时受到 Tailscale 网络限制和 Bearer 身份验证保护。
...
- NestJS API：`5000`。
- RAG API：`8000`。
- PostgreSQL 16 或更高版本可以在 Docker 中运行，也可以作为 Windows 原生实例运行。RightNow 的本地连接契约是 `localhost:15433`；具体运行机制不属于应用依赖。
- Docker 将主机端口 `15433` 映射到容器端口 `5432`。原生实例直接监听 `15433`。这两种方案使用相同的本地后端 `DATABASE_URL`。
- OpenClaw Gateway：`18789`，除非部署配置覆盖该值。
- OpenClaw Provisioner：默认为 `8787`，并且仅绑定到云端主机明确指定的 Tailscale IP。
