...
- Backend 统一以 `Asia/Shanghai` 解释“今天、今晚、明天、本周”等相对时间。Chat/OpenClaw system prompt 注入当前日期、时间、星期和时区；语义分类器接收固定本地日期时间、独立星期和时区字段。
- 业务记录的权威时间来自 Backend/数据库 `createdAt` 与服务端生成的上海日期，客户端和模型不能覆盖权威时间。日志与审计继续使用可排序的服务器时间。
...
- L1/L2/L3 Chroma 分别持久化到 `/var/lib/rightnow/rag/l1`、`l2`、`l3`，均由 `rightnow` 服务用户管理。知识源以 release 中的 `rag-service/knowledge/l1-faq/`、`l2-core/`、`l3-books/` 为权威，索引可以重建。
...
- 反注册不直接删除目录：先写入不含正文/Token 的 `moving` manifest，再将 workspace 与 OpenClaw 2026.3.24 的 `/agents/<agentId>` state 原子 rename 到同盘 `OPENCLAW_QUARANTINE_ROOT/<operationId>`，最后将 manifest 标记为 `quarantined`，再原子移除唯一 Agent 配置并重启/等待 Gateway。重复相同 operation 返回幂等结果；`moving` 或无 manifest 的旧 orphan operation 必须按服务端推导的两个固定资源恢复执行；Gateway/配置失败时恢复 active 目录和 Agent 配置。
...
- 上传 quarantine 根目录由 `ACCOUNT_DELETION_UPLOAD_QUARANTINE_ROOT` 在服务端固定，必须位于 active uploads 目录之外。只允许当前用户 UploadAsset 映射到 uploads 根内的普通非符号链接文件；外部对象存储资产在对应 provider 删除适配器实现前安全失败。manifest 不包含文件正文，目标文件名包含源路径摘要以避免冲突；读取 manifest 时必须重新校验 source、destination、普通文件和非 symlink 边界，并以 `account-delete-<uuid>` operation ID 支持幂等隔离和恢复。
- Account deletion Worker 默认关闭，只能通过 `ACCOUNT_DELETION_WORKER_ENABLED=true` 显式启用。单实例内使用 busy gate 串行 drain；`REQUESTED/FAILED_RETRYABLE` 可立即 claim，`EXTERNAL_CLEANUP/EXTERNAL_QUARANTINED/DB_PURGE/FINALIZING` 仅在租约超过 5 分钟未刷新后可接管。持有者每 30 秒刷新 `updatedAt`，防止多个 Backend 实例同时执行同一 Job。外部步骤固定顺序为 Provisioner Agent/workspace/session quarantine，再执行 Upload quarantine；两者使用同一 `externalOperationId`。
...
- Provisioner 提供 Bearer 认证 `GET /quarantine` 和 `POST /quarantine/purge`。列表仅返回 operationId、agentId、状态、隔离时间、年龄和资源数量，不返回路径或内容。purge 只接受服务端 quarantine root 下的单 operation、1-3650 天保留期和显式 dryRun；未到期时即使 dryRun=false 也不删除。apply 采用 `.purging` 原子 claim、`_purged` tombstone 和递归删除；重试必须继续清理仅有 `.purging`、或同时存在 tombstone 与 `.purging` 的中间状态，只有 tombstone 存在且数据目录已消失时才返回 alreadyPurged。
...
- V2 首批范围为 `today_plan`、`weekly_plan`、`today_todos` 和 `pending_todos` 四个低风险只读路由。同步语义执行和多轮指代尚未启用。
- 低风险确定性只读范围另包括 `today_diet`、`training_history`、`latest_weight` 和 `current_progress`。八个路由都在远程分类、OpenClaw、RAG 和 Memory 同步之前短路，且只允许当前 JWT userId 的 PostgreSQL 查询。
...
- Shadow runner 支持 `SHADOW_PER_GROUP_LIMIT`、`SHADOW_CONCURRENCY` 和 `SHADOW_MAX_P95_MS`。默认门禁要求有效率 `>=99%`、完全匹配率 `>=93%`、P95 `<=5000ms`；任一失败均不得开启同步语义执行。
- `INTENT_CLASSIFIER_VERSION=v2-readonly` 只在确定性规则未命中时允许同步语义候选。执行模式固定 4500ms、单次尝试，只接受 low risk、confidence 达标、`requestedWrite=false` 且能由 Backend 映射到白名单 route 的 query；唯一首批 analyze 是 `progress/analyze/current`。连续 5 次错误熔断 60 秒并回退旧链路。
...
- `rag-service/knowledge/l1-faq/faq.json`：权威的结构化 FAQ 来源；每个条目都必须具有唯一 ID、问题和答案。
- `rag-service/knowledge/l2-core/`：权威的精炼核心指导 Markdown 来源；导入时记录源文件名。
- `rag-service/knowledge/l3-books/`：权威的深度参考以及安全/恢复 Markdown 来源；与 L2 保持分离，以便进行明确的风险路由。
