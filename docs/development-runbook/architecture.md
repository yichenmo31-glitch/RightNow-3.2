...
- 账户删除采用持久状态机而不是跨 PostgreSQL/文件系统伪事务。User 先从 `ACTIVE` 转为 `DELETION_PENDING` 并递增 `authVersion`，JWT guard 立即拒绝 pending 或旧版本 Token；同一事务撤销 Agent/微信绑定入口并创建不依赖 User FK 的 `AccountDeletionJob`。
