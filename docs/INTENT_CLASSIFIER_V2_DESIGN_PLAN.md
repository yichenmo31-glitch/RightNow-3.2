...
当前自动观测门禁为：有效响应率 `>=99%`、有效结果三字段完全匹配率 `>=93%`、P95 `<=5000ms`。`v2-readonly` 仅在确定性规则未命中时同步尝试语义分类，执行超时 4500ms、单次尝试；只允许低风险、高置信、无写入的白名单 query，以及 `progress/analyze/current`。连续 5 次失败熔断 60 秒。写入不随该模式开放。
